import { Travel, TravelStatus, CreateTravelData, UpdateTravelData } from '../dtos/TravelDto'
...
class TravelService {
  ...
  async getTravelItinerary(travelId: string): Promise<ActivitySection[]> {
    try {
      ...
      console.error(`ActivitySection API Error: Failed to fetch itinerary for travel ${travelId}`, error);
  ...
  // Update travel
  async updateTravel(id: string, travelData: UpdateTravelData): Promise<Travel> {
    try {
