...
const cafeColor = activityIcons.find((icon) => icon.name === ActivityType.cafeRestaurant)?.color || "#FF5722";
  ...
  return (
    <View className="flex-row items-start mb-3 gap-6">
      {icon ? (
        ...
        >
          <Icon name={icon as any} size={16} color={cafeColor + "95"} />
        </View>
      ) : null}
        ...
        {onPress ? (
          <TouchableOpacity onPress={onPress} activeOpacity={0.7} accessibilityRole="button" className="flex-row items-center gap-1">
            <Text
              ...
              style={{
                color: isLink ? cafeColor : "#182230",
                textDecorationLine: isLink ? "underline" : "none",
              }}
            >
              {value}
            </Text>
            {isLink && <Icon name="open-in-new" size={18} color={cafeColor + "80"} />}
          </TouchableOpacity>
        ) : (
  ...
  return (
    <View className="rounded-3xl mb-6 shadow-md overflow-hidden">
      ...
      <View
        ...
        style={{ backgroundColor: `${cafeColor}1A` }}
      >
        <View className="flex-row items-center gap-2">
          <Icon name="restaurant" size={20} color={cafeColor} />
      ...
      <View className="p-4 border-l-2 border-r-2 border-gray-500 bg-white">
        <View className="mb-2">
          ...
          <Text className="text-3xl font-extrabold tracking-tight mb-1" style={{ color: cafeColor }}>
