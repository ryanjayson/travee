...
const EditTravelItinerary = forwardRef<EditTravelItineraryRef, EditTravelItineraryProps>(({
    ...
    setSections((prevSections) =>
      travelSections?.map((section) => {
        ...
        return {
          ...
          isCollapsed: existingSection !== undefined
            ? existingSection.isCollapsed
            : section.isCollapsed,
        };
      }) ?? [],
    );
  ...
  const handleMasterSectionDragMove = (currentIndex: number, dy: number) => {
  ...
  return (
    ...
    >
      <View className="mb-5">
        ...
        .filter(section => section.isDefaultSection == true)
        .map((section) => (
          <View
            key={section.id}
            style={[
            ...
            .map((section, mapIndex) => (
              ...
              >
                {(panHandlers, isSectionActive) => (
                  <View>
                  <View
                    className="bg-white rounded-lg p-2.5 border border-[#DDD] mb-4"
                    style={[
                      {
                        zIndex: sectionDragState?.sectionId === section.id ? 999 : 1,
                        elevation: sectionDragState?.sectionId === section.id ? 10 : 1,
                ...
                {!section.isCollapsed && (
                  <TouchableOpacity
                    ...
                    className="mt-2 h-[64px] flex items-center justify-center flex-row"
                  >
